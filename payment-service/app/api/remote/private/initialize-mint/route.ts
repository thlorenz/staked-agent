...
import { buildInitializeMint } from "@/src/server/magicblock";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const response = await buildInitializeMint(config, {
      owner,
      payer,
    ...
    return jsonError(500, "Unable to build initialize mint transaction", details);
  }
}
