...
import { performCustodialPayment } from "@/src/server/payments/custodial-pay";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const response = await performCustodialPayment(loadConfig(), body);
    ...
    return jsonError(500, "Unable to complete payment", details);
  }
}
