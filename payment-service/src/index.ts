...
function createApp(
  ...
  sender: ReturnType<typeof loadKeypairFromFile>,
...
try {
  ...
  const app = createApp(config, sender, connection);
  ...
  app.listen(config.port, () => {
    ...
    console.log(
      `payment-service using sender ${sender.publicKey.toBase58()} and RPC ${config.solanaRpcUrl}`,
    );
  });
} catch (error) {
  console.error(
